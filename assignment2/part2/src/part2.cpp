...
void drawMesh(Context &ctx, GLuint program, const MeshVAO &meshVAO)
{
	...
	glm::mat4 view = glm::lookAt(eyePos, origin, up);
	glm::mat4 projection = glm::perspective(90.0f, (float)ctx.width / ctx.height, 0.1f, 100.0f);
	...
	glUniform3fv(glGetUniformLocation(ctx.program, "u_light_position"), 1, &lightPos[0]);
