...
void createCube(Context &ctx)
{
	...
	*/
	...
	glGenBuffers(1, &ctx.indexVBO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,ctx.indexVBO);
...
void drawCube(Context &ctx)
{
	...
	glm::vec3 cube_center = glm::vec3(cos(elapsed_time),sin(elapsed_time),0.0f);
	glm::vec3 eye = glm::vec3(0.0f,0.0f,2.0f);
	glm::vec3 up = glm::vec3(0.0f,1.0f,0.0f);
    // Define the model, view, and projection matrices here
	glm::mat4 model = glm::translate(glm::mat4(1.0f),cube_center) * glm::rotate(glm::mat4(1.0f), (float)elapsed_time, glm::vec3(0,1,0));
	glm::mat4 view = glm::lookAt(eye,glm::vec3(0),up);//glm::mat4(1.0f);
	glm::mat4 projection = glm::perspective(90.0f,(float)ctx.width/ctx.height,0.1f,100.0f);//glm::mat4(1.0f);
